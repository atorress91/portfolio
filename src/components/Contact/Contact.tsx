    ...
    >
      ...
      <div className="row justify-content-center flex-grow-1">
        <div className="col-12 col-lg-10 d-flex flex-column">
          <h3 className={`${styles.sectionHeadText} text-center mt-lg-2 mb-lg-5 mb-2`}>{t('title') || 'Contact.'}</h3>
                ...
                <form ref={formRef} onSubmit={handleSubmit}>
                  <div className="row">
                    <div className="col-md-6 mb-3">
                      <label className={`mb-2 ${styles.formLabel}`}>{t('nameLabel') || 'Your Name'}</label>
                      <input
                        type="text"
                        name="name"
                        value={form.name}
                        onChange={handleChange}
                        placeholder={t('placeholderName') || "What's your name?"}
                        className={`form-control ${styles.customInput}`}
                        required
                      />
                    </div>
                    <div className="col-md-6 mb-3">
                      <label className={`mb-2 ${styles.formLabel}`}>{t('emailLabel') || 'Your Email'}</label>
                      <input
                        type="email"
                        name="email"
                        value={form.email}
                        onChange={handleChange}
                        placeholder={t('placeholderEmail') || "What's your email?"}
                        className={`form-control ${styles.customInput}`}
                        required
                      />
                    </div>
                  ...
                  <div className="mb-3">
                    <label className={`mb-2 ${styles.formLabel}`}>{t('messageLabel') || 'Your Message'}</label>
                    <textarea
                      rows={5}
                      name="message"
                      value={form.message}
                      onChange={handleChange}
                      placeholder={t('placeholderMessage') || 'What do you want to say?'}
                  ...
                  <div className="d-flex justify-content-between align-items-center mt-4">
                    <div className={`${styles.contactInfo}`}>
                      <p className="mb-0 d-flex align-items-center">
                        ...
                        <span>{t('contactMessage') || "I'll get back to you as soon as possible"}</span>
                      </p>
                    </div>
                    <motion.button
                      type="submit"
